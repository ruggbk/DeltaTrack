...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function findBlockOf(node) {
    ...
    while (el && FIND_INLINE[el.tagName]) el = el.parentElement;
    return el;
  }
  ...
  //
  // Three normalizations, all mirroring what the parser already does to produce
  // the merged (whole-word) canonical text:
  //   - soft hyphens rejoined  (`WORD-` + lowercase continuation, the rule in
  //     parsers/pdf_text.py `_merge_print_lines`; an uppercase continuation is a
  //     real compound like `Child-Rescue` and is left alone)
  ...
  function buildFindIndex(root) {
    var walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode: function(node) {
        ...
        if (el.closest('.fb-gutter, .fb-page')) return NodeFilter.FILTER_REJECT;
        if (el.offsetParent === null && el.tagName !== 'BODY') return NodeFilter.FILTER_REJECT;  // filter-hidden
    ...
    var parts = [], pieces = [], flatLen = 0, lastCh = '', block = null, node;
