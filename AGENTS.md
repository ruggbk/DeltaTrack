...
- **Branch from `develop`** (never `main`), commit in small focused steps, and open the PR against `develop`. A maintainer merges; do not merge yourself. Branch off `develop` itself rather than off another feature branch — see CONTRIBUTING's "Branch workflow" for why a stacked PR can read MERGED without reaching `develop`.
...
- Bill XML has structural containers nested inside titles: `subtitle`, `part`, `chapter`, `subchapter`, `subpart` (the HOLC higher-unit ladder below title). These are handled by `_walk_structural_children()` in `bill_tree.py`, which recurses through them to reach sections and appropriations elements.
...
- **Every content-skip in those three gates** (`test_corpus_properties`, `test_corpus_tree_properties`, `test_diff_validation` — listed in `CORPUS_GATE_MODULES`) **must be allowlisted** in `ALLOWED_CORPUS_SKIPS` (`tests/conftest.py`, #220). The manifest floor proves fixtures are committed and cases collected, but not that any assertion *ran* — a parser regression turning every case into a content-skip ("no bill body", "no anchors") would otherwise keep CI green while asserting nothing. A `pytest_sessionfinish` hook fails the session on any unlisted skip. If you add a fixture the gates cannot assert on, add it with a comment saying why: each entry records a coverage gap, not a neutral fact. The ceiling watches only these three because they skip per-case on content; the migrated corpus modules (`test_node_join_corpus` etc.) hard-assert denominators instead of skipping, so there is nothing there to watch today — add them to `CORPUS_GATE_MODULES` if that ever changes.
- **Every corpus gate is now on the committed manifest and fails closed with no env var** (#220). `test_node_join_corpus`, `test_xml_subsection_nodes` and `test_pdf_subsection_recall` used to parametrize over fetched (uncommitted) bills and so ran *empty and green* on an unfetched checkout, including CI (fail-open, #167). Their fixtures are committed and manifested, and each carries a `test_manifest_fixtures_committed` floor. `require_corpus_or_skip` and `REQUIRED_CORPUS_BILLS` are gone; don't reintroduce an `.exists()` filter on a parametrization list, which is the shape that fails open.
- `REQUIRE_CORPUS=1` survives but no longer means "make the corpus gates strict". It now gates only the two things that genuinely need a fetched corpus *and a network*: `test_govinfo_corpus_parity` (a live BILLSTATUS fetch per bill, and the only check that the README's documented setup path still matches the code — #271) and `test_validate_extraction`'s floor over the fetched Legislative Branch validation set.
- **A git worktree is fail-open for the remaining fetched-bill suites** (the live parity gate, the Legislative Branch spreadsheet validation, and the optional PDF suites): `bills/`'s uncommitted tier and `bills_corpus/` are gitignored, so they don't propagate to a new worktree even though the main checkout has them fetched. All *manifest* fixtures do propagate (they're in git), so every corpus gate now runs correctly in a worktree. For the rest, the tell is the collected/deselected count: compare it against a main-checkout run before trusting a green.
