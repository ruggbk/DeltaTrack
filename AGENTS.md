...
- Fetch tooling is layered: `fetch_bills.py` (per-bill text; `--source` defaults to keyless **govinfo**, `--source api` for the Congress.gov API — plus `search` for keyless title discovery over the local BILLSTATUS index), `fetch_bill_archives.py` (bulk bill *metadata* from govinfo BILLSTATUS archives; produces the ZIPs `search` reads), and `fetch_bill_text_archives.py` (bulk per-`(congress, type)` bill *text* download into `bills/`). `fetch_bills.py` and `fetch_bill_archives.py` share `shared/` (`http.py` API client + retry, `bill_types.py` the bill-type vocab, `version_stems.py` the version-file resolver) and `bill_index/` (a CSV-backed `BillIndex` keyed by `{congress}-{type}-{number}` slug; `parse_bill_id`/`make_bill_id`). `download-all --file <csv>` reads a slug list through `BillIndex`. Bills are stored as `bills/{congress}-{type}-{number}/{n}_{label}.{ext}` (folder = bill, file = version).
