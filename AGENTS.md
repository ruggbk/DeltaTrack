...
- **A git worktree is the same fail-open, and it is easier to miss:** `bills/` and `bills_corpus/` are gitignored, so they do not propagate to a new worktree even though the main checkout has them fetched. A bare `pytest` there collects roughly half the suite and reports green. Run verification gates in the main checkout, or `REQUIRE_CORPUS=1` in the worktree (it fails loudly there and passes in the main checkout — verified both ways). The tell is the collected/deselected count: compare it against a main-checkout run before trusting a green.
