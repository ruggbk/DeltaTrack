...
| `probe_node_identity.py` | Whether an ADR 0019 ordinal is still recoverable where matching runs: do `match_nodes` **and** `apply_similarity_revocation` return the *same objects* the source `BillTree.nodes` holds, and is that recovery a bijection across the whole pre-classification sequence? Carries three negative controls, because a lookup that cannot miss proves nothing — a value-equal copy substituted for the real node, value collisions within one tree, and output position used as if it were the ordinal. The position figure is also a tripwire: it must come out equal at both stages, since the revocation stage replaces a pairing *in place* and canonical output depends on that. |
| `probe_round2_migration.py` | What a pre-classification second retrieval round would have to reproduce. Measures the part `probe_splits.py` leaves open — how much of the move-candidate and selected-move population depends on an observation **the similarity rule left unmatched** — and puts the legacy `(ri, ai)` sort key and the ADR 0019 ordinal key through one greedy loop to see whether they select the same correspondence. Reads the revocation population from production's own predicate and cross-checks it against what the revocation stage structurally did. Refuses to report any figure unless that cross-check holds and its duplicated loop first reproduces production's selected set *and* order. Since #591 the population figure sizes an *ordering constraint inside assignment* (round 2 must run after the revocation stage), not a dependency on classification output. |
