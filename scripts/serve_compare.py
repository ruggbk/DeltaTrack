...
from deltatrack.formatters.canonical import view_from_canonical, xml_diff_to_canonical  # noqa: E402
...
def _render_xml_diff_html(v1_path: Path, v2_path: Path) -> str:
    ...
    for key, stem in (("old_version_number", v1_path.stem), ("new_version_number", v2_path.stem)):
        ...
        if num is not None:
            diff_dict[key] = num
    full_text, full_text_spans, tree = build_xml_full_text(v1, v2)
    canonical = xml_diff_to_canonical(diff_dict, full_text=full_text, full_text_spans=full_text_spans, tree=tree)
    return format_diff_html(view_from_canonical(canonical), canonical=canonical, title=bill_title(v2))
