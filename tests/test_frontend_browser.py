...
#: Every collapsible on the page, as (CSS selector for the element the caret hangs
#: off, human name). The caret is a ``::before`` on each.
_CARET_OWNERS = [
    (".nav-group > summary", "sidebar navigation group"),
    (".card-group > summary", "change card group"),
    (".financial-summary > summary h2", "Financial Summary heading"),
]


def test_every_caret_is_sized_against_the_text_it_opens(chromium, tmp_path):
    """A disclosure caret scales with its own label, not a fixed pixel size.

    The page carries collapsibles at three text sizes: 13px sidebar navigation,
    16px card group headings, and the 24px serif Financial Summary heading. A
    caret pinned to one number reads as a control at the size it was chosen for
    and as decoration everywhere else, which is what made the Financial Summary
    heading not look clickable at all. Sizing every caret in `em` against its
    label keeps the affordance proportionate wherever it appears.

    Asserted as the caret-to-label ratio because that is the invariant. Pinning
    the pixel sizes would re-encode the numbers this rule exists to remove, and
    would fail on an unrelated type-scale change.
    """
    report = tmp_path / "carets.html"
    ...
    measured = page.evaluate(
        """(selectors) => selectors.map(sel => {
            ...
            return {
                ...
                caret: parseFloat(getComputedStyle(el, '::before').fontSize),
            };
        })""",
        [sel for sel, _ in _CARET_OWNERS],
    )
    page.close()

    ratios = {}
    for (sel, name), m in zip(_CARET_OWNERS, measured):
        assert not m.get("missing"), f"{name} ({sel}) did not render; the measurement below would be vacuous"
        ratios[name] = (m["caret"] / m["label"], m["caret"], m["label"])

    # A per-caret band is too weak on its own: a caret pinned to 10px still lands
    # inside any sane band next to 13px sidebar text, so the regression that
    # matters (one caret pinned while the type scale moves around it) passes. What
    # the rule actually says is that every caret holds the *same* proportion to its
    # own label, so the spread between them is the thing to assert.
    spread = max(r for r, _, _ in ratios.values()) - min(r for r, _, _ in ratios.values())
    assert spread <= 0.05, "carets are not in proportion to the text they open: " + ", ".join(
        f"{name} {caret:.1f}px/{label:.1f}px = {r:.2f}" for name, (r, caret, label) in ratios.items()
    )

    # And the shared proportion has to be a readable one, not uniformly tiny.
    assert 0.7 <= min(r for r, _, _ in ratios.values()) <= 1.0, f"caret proportion out of range: {ratios}"

    # The spread check only means something across a range of label sizes: if every
    # collapsible rendered at one size, a hardcoded px caret would satisfy it too.
    seen_labels = {round(label) for _, _, label in ratios.values()}
    assert len(seen_labels) >= 2, f"all carets measured at the same label size {seen_labels}; the check is vacuous"
