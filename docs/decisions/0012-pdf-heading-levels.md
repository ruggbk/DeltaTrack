...
[0005](0005-deltatrack-boundary.md) / [0010](0010-pdf-pipeline-pre-publication.md)).
...
- **An XML cross-reference to label the PDF levels.** Rejected as automatic
  ...
  forbids ([0005](0005-deltatrack-boundary.md) / [0008](0008-deterministic-engine.md)).
  It counts only when the user supplies the XML — which is just the XML pipeline — or
  as a concern for a consuming tool.
