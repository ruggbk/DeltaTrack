...
- Commands, modules, APIs, types and schema fields describe the job or concept they
  actually represent.
- Nouns identify concepts, results and capabilities; verbs identify operations.
...
- **The umbrella term for the substantive comparison domain.** `bill` is established as
  ...
  considered all remain open. That decision belongs to **ADR 0006** and the
...
- **Whether the canonical contract's `bill.type` is correct.** The spike established
  ...
  govern that field in a pipeline-neutral contract. **ADR 0006's question.**
...
- **Parts of the current surface do not comply, and this record does not schedule their
  ...
  acquisition umbrella and the `<bill>` document type. Each belongs to the record that owns
  the surface — 0006, 0013, or a future change — and none is licensed by this record alone.
...
- [ADR 0006](0006-canonical-diff-contract.md) — the canonical contract, `schema_version`, and the owner of the domain-vocabulary question this record defers
