...
contract: uploaded bill data does not leave the user's local machine. The user provides
the two versions; the tool produces a self-contained report.
...
1. Is the feature **deterministic and reproducible**? If not, it should be considered
   out of scope.
2. Does the feature **require bill data to leave the user's local environment**? For
   example, does it require an external service? If so, it is out of scope.
...
- Every "does this belong in DeltaTrack?" question now has a default answer: if it is not
  deterministic, exposes the user's bill version data, or compares more than 2 versions of
  a bill, it is out of scope.
