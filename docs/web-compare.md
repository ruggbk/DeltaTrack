...
| **Process on our server** (`compare.html`, right card) | **Available now** (interim) | User uploads start + end PDFs; server runs the Python PDF diff and returns a standalone HTML report in a new tab. Stateless — PDFs are not stored. **Known interim exception to ADR 0011 (local-only processing)**: uploads leave the user's machine, so the page warns against non-public drafts. Retirement is tracked in [#112](https://github.com/AgoraDMV/DeltaTrack/issues/112). |
